...
export const currentUser = async (req, res) => {
  try {
    ...
    const user = await UserModel.findById(req.user).select("-password"); // It means verified user only
...
export const deleteUser = async (req, res) => {
  try {
    // req.user is coming from verifyToken - decoded the token
    const user = await UserModel.findByIdAndDelete(req.user); // It means verified user only
    if (!user) {
      return res
        .status(404)
